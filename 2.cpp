...
int main() {
    sf::RenderWindow window(sf::VideoMode(1920, 1080), "Robot Delivery Miami - ROBOT CORREGIDO", sf::Style::Fullscreen);
    ...
    if (!mapTexture.loadFromFile("img/miami_map.png")) {
        std::cout << "❌ ERROR: No se pudo cargar img/miami_map.png" << std::endl;
    } else {
    ...
    robot.setScale(0.05f, 0.05f);
    robot.setOrigin(tex.getSize().x/2.f, tex.getSize().y/2.f);
    robot.setPosition(960.f, 540.f);
    
    // 🔄 VOLTEAR HORIZONTALMENTE el robot
    robot.setScale(-0.05f, 0.05f); // Escala negativa en X para voltear
    ...
    while (window.isOpen()) {
        ...
        if (mapLoaded) window.draw(mapBackground);
        window.draw(robot);
